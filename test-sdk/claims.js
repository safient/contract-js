...
const { ClaimType, ClaimAction } = require('../dist/types/Types');
...
describe('safientMain', async () => {
  ...
  describe('Safient Claims Test Flow', async () => {
    ...
    it('Should allow safe creator to SIGNAL when the safe is claimed', async () => {
      ...
      await safientMain.sendUpdateSafe(safeId[2],ClaimType.SignalBased,0,'',ClaimAction.Signal);
    ...
    it('Should allow beneficiaries to create a claim (D-Day based)', async () => {
      ...
      await safientMainCreator.createSafe(beneficiaryAddress, safeId[3], Types.ClaimType.DDayBased, 6, '', '');
    ...
    it('Should allow safe current owner to update the D-Day', async () => {
      ...
      // SUCCESS : create another safe with safeId4 (for claimType - DDayBased) with DDay set to 6 seconds
      await safientMainCreator.createSafe(beneficiaryAddress, safeId[4], Types.ClaimType.DDayBased, 6, '', '');
      ...
      await safientMainCreator.sendUpdateSafe(safeId[4], ClaimType.DDayBased, now + 12, '',ClaimAction.Dday); // update the D-Day to 12 seconds from the time of updating
    ...
    it('Should allow beneficiaries to create a claim (E-Day based)', async () => {
      const safientMainCreator = new SafientMain(safeCreatorSigner, chainId);
      const safientMainBeneficiary = new SafientMain(beneficiarySigner, chainId);
      const safientMainAccountX = new SafientMain(accountXSigner, chainId);
      ...
      // SUCCESS : create another safe with safeId4 (for claimType - DDayBased) with DDay set to 6 seconds
      await safientMainCreator.createSafe(beneficiaryAddress, safeId[5], Types.ClaimType.Expirion, 6, '', '');
      const mineNewBlock1 = new Promise((resolve, reject) => {
        setTimeout(() => {
          resolve(provider.send('evm_mine'));
        }, 7000);
      ...
      const result1 = await mineNewBlock1;
      // create a claim - after E-Day (claim should fail)
      await expect(safientMainBeneficiary.createClaim(safeId[5], '')).to.be.rejectedWith(Error);;

      // create a new safe - E-Day
      await safientMainCreator.createSafe(beneficiaryAddress, safeId[6], Types.ClaimType.Expirion, 6, '', '');

      // create a claim - before E-Day (claim should pass)
      const tx2 = await safientMainBeneficiary.createClaim(safeId[6], '');
      const txReceipt2 = await tx2.wait();
      const claimId2 = txReceipt2.events[0].args[1];
      const claimID2 = parseInt(claimId2._hex);

      // check claim status (DDayBased)
      const safeId4ClaimResult2 = await safientMainAccountX.getClaimStatus(safeId[6], claimID2);
      expect(safeId4ClaimResult2).to.equal(1); // claim got Passed (before E-Day)
    });

    it('Should allow safe current owner to update the E-Day', async () => {
      let latestBlockNumber, latestBlock, now;
      ...
      // SUCCESS : create another safe with safeId4 (for claimType - Expirion) with EDay set to 6 seconds
      await safientMainCreator.createSafe(beneficiaryAddress, safeId[7], Types.ClaimType.Expirion, 6, '', '');
      ...
      // create a claim - after E-Day (6 seconds) (claim should fail)
      await expect(safientMainBeneficiary.createClaim(safeId[7], '')).to.be.rejectedWith(Error);;


      // SUCCESS : create another safe with safeId8 (for claimType - Expirion) with EDay set to 6 seconds
      await safientMainCreator.createSafe(beneficiaryAddress, safeId[8], Types.ClaimType.Expirion, 6, '', '');

      latestBlockNumber = await ethers.provider.getBlockNumber();
      latestBlock = await ethers.provider.getBlock(latestBlockNumber);
      now = latestBlock.timestamp;

      // Update the E-Day - 12 seconds
      await safientMainCreator.sendUpdateSafe(safeId[8],ClaimType.Expirion, now + 12, '', ClaimAction.Eday); // update the E-Day to 12 seconds from the time of updating

      // create a claim - before E-Day (10 + 2 = 12 seconds) (claim should pass)
      const tx3 = await safientMainBeneficiary.createClaim(safeId[8], '');
      const txReceipt3 = await tx3.wait();
      const claimId3 = txReceipt3.events[0].args[1];
      const claimID3 = parseInt(claimId3._hex);

      // check claim status (DDayBased)
      const safeId4ClaimResult3 = await safientMainAccountX.getClaimStatus(safeId[8], claimID3);
      expect(safeId4ClaimResult3).to.equal(1); // claim got Passed (before E-Day)
    });
    ...
    it('Should allow beneficiaries to create a safe (SignalBased) and update to all claim types', async () => {
      ...
      const safientMain2 = new SafientMain(beneficiarySigner, chainId);

      // SUCCESS : create claim on safeId2
      tx = await safientMain2.createClaim(safeId[9], '');
      txReceipt = await tx.wait();
      claimIdOfSafeId1 = parseInt(txReceipt.events[0].args[1]._hex)

      const safeWithSafeId1 = await safientMain2.getSafeBySafeId(safeId[9]);
      expect(safeWithSafeId1.claimsCount).to.equal(1);

      const claimOnSafeId1 = await safientMain2.getClaimByClaimId(claimIdOfSafeId1);
      expect(claimOnSafeId1.claimedBy).to.equal(beneficiaryAddress);
      expect(claimOnSafeId1.status).to.equal(0);
      ...
      // SUCCSESS : update claim type to Dday
      await safientMain.sendUpdateSafe(safeId[9], ClaimType.DDayBased, now + 10, '', ClaimAction.Update);
      const safeWithSafeId3 = await safientMain.getSafeBySafeId(safeId[9]);
      expect(safeWithSafeId3.claimType).to.equal(ClaimType.DDayBased);
      latestBlockNumber = await ethers.provider.getBlockNumber();
      latestBlock = await ethers.provider.getBlock(latestBlockNumber);
      now = latestBlock.timestamp;
      // SUCCSESS : update claim type to Eday
      await safientMain.sendUpdateSafe(safeId[9], ClaimType.Expirion, now + 10, '', ClaimAction.Update)
      const safeWithSafeId4 = await safientMain.getSafeBySafeId(safeId[9]);
      expect(safeWithSafeId4.claimType).to.equal(ClaimType.Expirion);
    });

    it('Should allow beneficiaries to create a safe (SignalBased) and create claim and do not update claim', async () => {
      const safientMain = new SafientMain(safeCreatorSigner, chainId);
      ...
      // SUCCESS : create claim on safeId2
      tx = await safientMain2.createClaim(safeId[10], '');
      txReceipt = await tx.wait();
      claimIdOfSafeId1 = parseInt(txReceipt.events[0].args[1]._hex)

      const safeWithSafeId1 = await safientMain2.getSafeBySafeId(safeId[10]);
      expect(safeWithSafeId1.claimsCount).to.equal(1);
      ...
      // SUCCSESS : send signal
      await safientMain.sendUpdateSafe(safeId[10], ClaimType.SignalBased, 0, '', ClaimAction.Signal);
      const safeWithSafeId3 = await safientMain.getSafeBySafeId(safeId[10]);
      expect(safeWithSafeId3.claimTimeStamp).to.equal(0);
      ...
      // SUCCSESS : update claim type to Dday
      await safientMain.sendUpdateSafe(safeId[10], ClaimType.DDayBased, now + 10, '', ClaimAction.Update);
      const safeWithSafeId4 = await safientMain.getSafeBySafeId(safeId[10]);
      expect(safeWithSafeId4.claimType).to.equal(ClaimType.SignalBased);
    });
    ...
    it('Should allow beneficiaries to create a safe (SignalBased) and make deprecated and do not allow to claim or send signal', async () => {
      ...
      // SUCCSESS : send signal
      await safientMain.sendUpdateSafe(safeId[10], ClaimType.SignalBased, 0, '', ClaimAction.Deprecated);
      const safeWithSafeId3 = await safientMain.getSafeBySafeId(safeId[10]);
      expect(safeWithSafeId3.deprecated).to.equal(true);
      ...
      // SUCCSESS : update claim type to Dday
      safientMain.sendUpdateSafe(safeId[10], ClaimType.DDayBased, now + 10, '', ClaimAction.Update);
      const safeWithSafeId4 = await safientMain.getSafeBySafeId(safeId[10]);
      expect(safeWithSafeId4.claimType).to.equal(ClaimType.SignalBased);
    });
  });
